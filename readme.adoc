...
This is fine in most cases, but it has a specific drawback, in that, as configured, it doesn't pass through the IP address of the requesting clientfootnote:[Exposing apps with load balancers for VPC
...
In contrast to an ALB, a Network Load Balancer (NBL) does pass through the client's IP addressfootnote:[b].
...
But that's rarely the case, so we need another piece, a custom HTTP/S proxy to sit between the NLB and the application Pods, that will terminate the TLS and proxy traffic to the application's plain HTTP port.
...
At this point we would normally expose the deployment as a service, and then the service as a route.
...
Let's clean up our test service and route and move on.

[source,bash]
oc delete route echo-ip

Now that we've proven the need for an NLP, let's create one.
...
This is the core of what this project is about.
...
The Dockerfile is a multi-arch build that will build and push the image to your Docker Hub account.
